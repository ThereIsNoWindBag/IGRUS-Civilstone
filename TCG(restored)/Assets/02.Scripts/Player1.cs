	...
	void Update ()
    {
        ...
        if (flgBuyUnits == 2)                                                                      //유닛배치 flag 2
        {
            ...
            {                                                                                         //즉, material이 바뀔때 마다 한번씩만 호출된다.
                ...
                {
                    instantUnit = Instantiate(unitArcher, rayObj.transform.position + Vector3.up, Quaternion.identity) as GameObject;
                    instantUnit.tag = "Unit1";

                    groundName = back.name;

                    (instantUnit.GetComponent("playArcher") as playArcher).coorR = int.Parse(groundName.Substring(7, 1));
                    (instantUnit.GetComponent("playArcher") as playArcher).coorRU = int.Parse(groundName.Substring(9, 1));

                    notMovable[unitCount, 0] = int.Parse(groundName.Substring(7, 1));
                    notMovable[unitCount, 1] = int.Parse(groundName.Substring(9, 1));
                    playArcher.unitCountNum = unitCount;
